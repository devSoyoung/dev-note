...
## Mybatis
* 자바의 관계형 데이터베이스 프로그래밍을 좀더 쉽게 할 수 있도록 해주는 프레임워크
* JDBC는 다양한 API를 제공하지만, 세부적인 작업을 위해 작업별로 메소드를 호출해야 하므로 다수의 메소드를 호출하고 관련 객체를 해제해야하는 번거로움 존재
* Mybatis는 JDBC보다 좀 더 쉽게 사용하기 위해서 개발된 것
* 참고링크 : [Mybatis란 무엇인가](https://iotsw.tistory.com/77)
...
## 스프링 컨테이너
* **\<bean> 저장소에 해당하는 XML 설정파일을 참조해 빈의 생명주기를 관리**
* **BeanFactory** : 스프링 설정파일에 등록된 빈 객체를 생성/관리하는 기본적인 컨테이너의 기능을 제공, 컨테이너가 구동될 때 빈을 생성하지 않고, 클라이언트의 요청에 의해서만 객체를 생성
* **ApplicationContext** : BeanFactory의 기능에 더해 트랜잭션 관리, 다국어 처리 등 다양한 기능을 지원, 컨테이너가 구동하는 시점에 \<bean>에 등록되어 있는 클래스를 객체화(즉시 로딩 방식) => `GenericXmlApplicationContext`

## DI
* Dependency Injection, 의존(=객체 간의 결합관계, **생성**) 주입(외부에서 **전달**)
* **의존** : 하나의 객체에서 다른 객체의 변수나 메소드를 사용하는 것 (이용할 객체 생성, 생성된 객체의 레퍼런스 정보)
* 설정파일(.xml)을 통해서 객체 간의 의존관계를 설정 => 의존관계를 객체 자신이 아니라 외부의 조립기가 수행
* 각 클래스 사이에 필요로 하는 의존관계를 Bean설정 정보 바탕으로 컨테이너가 자동으로 연결  
* 객체가 직접 의존하고 있는 객체의 생성/검색을 할 필요가 없으므로 코드 관리가 용이해짐

### 의존 주입 방법
1. **XML을 통한 설정** - 생성자(`<constructor-arg>`)를 통한 전달, 속성(`<property>`)을 통한 전달
2. **어노테이션을 통한 주입** - @Autowired, @Resource
(**@Autowired, @Resource의 차이** : 빈 탐색 시 우선순위 기준)

* 참고링크 : [스프링의 시작, 프레임워크의 구성요소](https://asfirstalways.tistory.com/334)


## IoC
* Inversion of Control, 제어의 역전
* 객체의 생성을 코드에서 실행하는 것이 아니라, 컨테이너가 담당
* 객체 사이의 의존관계 또한 컨테이너가 담당
* DI를 통해 IoC를 구현함
...
### AOP 구현 방법
1. **XML 이용**
2. **어노테이션 이용** - @Aspect(클래스를 AOP에서 사용하겠다는 뜻), @Before(대상 메서드 실행 전에 이 메서드를 실행하라는 뜻)
...
## 웹서버/웹 애플리케이션 서버
* **웹서버** : 클라이언트로부터 HTTP 요청을 받아들이고, HTML같은 웹 페이지, 자원(그림, 스타일시트, 스크립트 등)을 보내주는 역할을 하는 프로그램 - **정적 데이터 처리**
* **웹 애플리케이션 서버** : HTTP를 통해 사용자 컴퓨터나 장치에 애플리케이션을 수행해 주는 미들웨어 - **동적 데이터 처리**

### 웹 애플리케이션 서버의 기능
* 프로그램 실행 환경과 데이터베이스 접속 기능 수행
* 여러 개의 트랜잭션을 관리
* 업무를 처리하는 비즈니스 로직 수행
...
* [Spring 기본 동작 순서 및 구조](https://server-engineer.tistory.com/253)
* [웹서버와 웹 애플리케이션 서버의 차이](https://bsnippet.tistory.com/47)
